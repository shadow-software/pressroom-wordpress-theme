...
for entry in "${ROUTES[@]}"; do
  ...
  start=$(date +%s%N)
  code=$(curl -s -o /tmp/shadow-software-digest-theme-for-wordpress-smoke.html -w "%{http_code}" --max-time 20 "$BASE$path" 2>/dev/null)
  end=$(date +%s%N)
  ms=$(( (end - start) / 1000000 ))
  size=$(wc -c < /tmp/shadow-software-digest-theme-for-wordpress-smoke.html 2>/dev/null || echo 0)
  ...
  if grep -qiE "fatal error|allowed memory size|maximum execution time" /tmp/shadow-software-digest-theme-for-wordpress-smoke.html 2>/dev/null; then
