...
done < <(find "$ROOT/shadow-software-digest-theme-for-wordpress" -name '*.php')
while IFS= read -r f; do
  python3 -c "import json,sys;json.load(open(sys.argv[1]))" "$f" 2>/dev/null || { red "   bad JSON: $f"; fail=1; }
done < <(find "$ROOT/shadow-software-digest-theme-for-wordpress" -name '*.json')
...
"$ROOT/scripts/local-smoke.sh" >/tmp/shadow-software-digest-theme-for-wordpress-smoke.log 2>&1 || {
  red "✗ smoke test failed:"
  tail -20 /tmp/shadow-software-digest-theme-for-wordpress-smoke.log
  exit 1
}
...
php "$ROOT/scripts/local-assert.php" >/tmp/shadow-software-digest-theme-for-wordpress-assert.log 2>&1 || {
  red "✗ correctness assertions failed:"
  grep -E "✗|FAILED" /tmp/shadow-software-digest-theme-for-wordpress-assert.log | head -10
  echo
...
echo "   ✓ $(grep -oE '[0-9]+ assertions passed' /tmp/shadow-software-digest-theme-for-wordpress-assert.log)"

# ------------------------------------------------------------------ deploy ----

echo
echo "════ DEPLOYING to $SITE (one site only)"
rsync -az --delete -e "$SSH" "$ROOT/shadow-software-digest-theme-for-wordpress/" "$HOST:/var/www/$SITE/public/wp-content/themes/shadow-software-digest-theme-for-wordpress/"
$SSH "$HOST" "chown -R nobody:nogroup /var/www/$SITE/public/wp-content/themes/shadow-software-digest-theme-for-wordpress"
echo "   ✓ uploaded"

$SSH "$HOST" "cd /var/www/$SITE/public && wp theme activate shadow-software-digest-theme-for-wordpress --allow-root" >/dev/null
echo "   ✓ activated"
...
probe() {
  ...
  start=$(date +%s%N)
  code=$(curl -s -o /tmp/shadow-software-digest-theme-for-wordpress-live.html -w "%{http_code}" --max-time 20 "$url" 2>/dev/null || echo 000)
  end=$(date +%s%N)
