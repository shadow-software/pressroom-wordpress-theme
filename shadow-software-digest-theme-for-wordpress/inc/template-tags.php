		...
		<span class="digest-utility__est">
			<?php if ( $founded ) : ?>
				...
				printf(
					...
					esc_html__( 'Est. %1$s', 'shadow-software-digest-theme-for-wordpress' ),
 ...
 *
 * So Digest prefers, in order: a locally-uploaded avatar (a "shadow_digest_avatar"
 ...
 *
 * Digest renders the form and nothing else. It stores no addresses and sends no
	...
	<aside class="digest-newsletter" aria-labelledby="<?php echo esc_attr( $id ); ?>">
		...
		<div class="digest-newsletter__signup">
			<?php if ( '' === $action ) : ?>
				 ...
				 * Digest renders nothing and, for an administrator who can fix it,
				 * says why.
				 */
				if ( current_user_can( 'edit_theme_options' ) ) :
					?>
					<p class="digest-newsletter__note">
						<?php esc_html_e( 'No signup endpoint is configured, so the form is hidden. Set one under Customizer → Digest → Newsletter, or turn the box off.', 'shadow-software-digest-theme-for-wordpress' ); ?>
					</p>
					<?php
				endif;
				?>
			...
			<?php else : ?>
				...
				<?php if ( '' !== $count ) : ?>
					<p class="digest-newsletter__count">
						...
						printf(
							...
							esc_html__( 'Join %s readers', 'shadow-software-digest-theme-for-wordpress' ),
							esc_html( $count )
						);
						?>
					</p>
				...
				<form class="digest-newsletter__form" action="<?php echo esc_url( $action ); ?>" method="post">
					<label class="screen-reader-text" for="<?php echo esc_attr( $id ); ?>-email">
						<?php esc_html_e( 'Your email address', 'shadow-software-digest-theme-for-wordpress' ); ?>
					</label>
					...
					<input
						...
						placeholder="<?php esc_attr_e( 'your@email.com', 'shadow-software-digest-theme-for-wordpress' ); ?>"
						autocomplete="email"
						required
					/>

					<button class="digest-newsletter__button" type="submit">
						<?php esc_html_e( 'Subscribe', 'shadow-software-digest-theme-for-wordpress' ); ?>
					</button>
				</form>
