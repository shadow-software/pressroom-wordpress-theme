...
function shadow_digest_block_category( array $categories ): array {
	array_unshift(
		...
		array(
			'slug'  => 'shadow-digest',
			'title' => __( 'Pressroom — editorial', 'pressroom' ),
			'icon'  => null,
		)
	);
...
function shadow_digest_render_toc( array $attributes ): string {
	...
	return sprintf(
		...
		get_block_wrapper_attributes(),
		esc_attr__( 'Table of contents', 'pressroom' ),
		esc_html( $label ),
...
function shadow_digest_render_sources( array $attributes ): string {
	...
	$label = isset( $attributes['label'] ) && '' !== $attributes['label']
		? (string) $attributes['label']
		: __( 'Sources & References', 'pressroom' );
...
function shadow_digest_render_lead_body( array $attributes ): string {
	...
	$more = isset( $attributes['moreText'] ) && '' !== $attributes['moreText']
		...
		: __( 'Continue the full report →', 'pressroom' );
...
function shadow_digest_render_disclosure_table( array $attributes ): string {
		...
		if ( '' !== $row['partner'] ) {
			$partner = '' !== $row['url']
				? sprintf(
					...
					esc_html__( '(opens in a new tab, sponsored link)', 'pressroom' )
				)
				: sprintf(
	...
	$note = isset( $attributes['disclosure'] ) && '' !== trim( wp_strip_all_tags( (string) $attributes['disclosure'] ) )
		...
		: sprintf(
			...
			__( 'Affiliate disclosure: %s may earn a commission on purchases made through partner links. Commissions never influence which products we recommend; testing and editorial are handled independently of commercial partnerships.', 'pressroom' ),
