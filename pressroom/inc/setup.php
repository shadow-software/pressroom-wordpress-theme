...
function shadow_digest_setup(): void {
	/*
	 * Translations. Pressroom ships a .pot in languages/ and honours any .mo a
	 * translator drops in, or a translation installed from translate.wordpress.org.
	 */
	load_theme_textdomain( 'pressroom', SHADOW_DIGEST_PATH . 'languages' );
 ...
 * Pressroom registers classic locations as well so a publisher migrating an
...
function shadow_digest_register_menus(): void {
	register_nav_menus(
		array(
			'sections' => __( 'Section navigation (masthead)', 'pressroom' ),
			'utility'  => __( 'Utility bar (subscribe, sign in)', 'pressroom' ),
		)
	);
}
 ...
 *
 * Pressroom deliberately ships exactly one stylesheet and no front-end JavaScript
...
function shadow_digest_enqueue_assets(): void {
	...
	wp_add_inline_style( 'pressroom', shadow_digest_custom_properties() );
}
 ...
 *
 * Pressroom defines no duotones, so the filter definitions WordPress prints in the
...
function shadow_digest_register_sidebars(): void {
	register_sidebar(
		array(
			'name'          => __( 'Front page rail', 'pressroom' ),
			'id'            => 'sidebar-1',
			'description'   => __( 'The narrow left-hand column on the front page, beside the lead story. Traditionally short news briefs.', 'pressroom' ),
 ...
 *
 * OPTIONAL: this is a no-op on a site with no shop. Pressroom depends on no plugin
 ...
 * offers `woocommerce_get_image_size_<size>` to override it in code. Pressroom sets
 ...
 * to edit; that was checked before writing this). On Pressroom it surfaced as a
 * stray, unstyled person-icon floating below the section navigation, outside the
 * masthead's grid and belonging to nothing.
 *
 * Pressroom already renders account and cart links, deliberately placed and styled
 ...
 * WooCommerce only ever auto-inserts it into the header, and Pressroom's own header
