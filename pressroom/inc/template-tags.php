		...
		<?php if ( $account_url ) : ?>
			<a class="digest-shop__link" href="<?php echo esc_url( $account_url ); ?>">
				...
				echo is_user_logged_in()
					? esc_html__( 'My Account', 'pressroom' )
					: esc_html__( 'Log In', 'pressroom' );
				?>
			</a>
		<?php endif; ?>
 ...
 *
 * So Pressroom prefers, in order: a locally-uploaded avatar (a "shadow_digest_avatar"
 ...
 *
 * Pressroom renders the form and nothing else. It stores no addresses and sends no
	...
	<aside class="digest-newsletter" aria-labelledby="<?php echo esc_attr( $id ); ?>">
		...
		<div class="digest-newsletter__signup">
			<?php if ( '' === $action ) : ?>
				 ...
				 * Pressroom renders nothing and, for an administrator who can fix it,
				 * says why.
				 */
				if ( current_user_can( 'edit_theme_options' ) ) :
					?>
					<p class="digest-newsletter__note">
						<?php esc_html_e( 'No signup endpoint is configured, so the form is hidden. Set one under Customizer → Pressroom → Newsletter, or turn the box off.', 'pressroom' ); ?>
					</p>
					<?php
				endif;
				?>
			...
			<?php else : ?>
				...
				<?php if ( '' !== $count ) : ?>
					<p class="digest-newsletter__count">
						...
						printf(
							...
							esc_html__( 'Join %s readers', 'pressroom' ),
							esc_html( $count )
						);
						?>
					</p>
				...
				<form class="digest-newsletter__form" action="<?php echo esc_url( $action ); ?>" method="post">
					<label class="screen-reader-text" for="<?php echo esc_attr( $id ); ?>-email">
						<?php esc_html_e( 'Your email address', 'pressroom' ); ?>
					</label>
					...
					<input
						...
						placeholder="<?php esc_attr_e( 'your@email.com', 'pressroom' ); ?>"
						autocomplete="email"
						required
					/>

					<button class="digest-newsletter__button" type="submit">
						<?php esc_html_e( 'Subscribe', 'pressroom' ); ?>
					</button>
				</form>
