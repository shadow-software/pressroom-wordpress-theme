 ...
 * engines understand it, so Pressroom emits NewsArticle, Organization and
 ...
 * or All in One SEO is running, they own the graph and Pressroom gets out of the way.
 *
 * @package Pressroom
 * @since   1.0.0
 */
...
/**
 * Whether Pressroom should emit structured data at all.
 *
 * @since 1.0.0
...
function shadow_digest_should_emit_schema(): bool {
	...
	/**
	 * Filters whether Pressroom emits its own JSON-LD.
	 *
 ...
 *
 * Stock WordPress prints no meta description at all. Pressroom ships no SEO
 ...
 * front page, a category or author archive, or their paginated pages. Pressroom
...
function shadow_digest_meta_description(): string {
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		return sprintf(
			...
			__( 'Articles filed under %s.', 'pressroom' ),
	...
	if ( is_search() ) {
		return sprintf(
			...
			__( 'Search results for %s.', 'pressroom' ),
			get_search_query()
		);
	}
